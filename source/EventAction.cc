...
void EventAction::EndOfEventAction(const G4Event *) {
  ...
  G4long nTrackSMX[8], nTrackSMY[8], nTrackSM[8], nTrackSMXw[8], nTrackSMYw[8],
      nTrackSMw[8];
  ...
  for (G4int i = 1; i < 2; i++) {
    if (muonNEVOD[i][0][0] >= 0 && muonNEVOD[i][0][1] >= 0) {
      ...
      muonTrackLengthNEVOD +=
          sqrt((muonNEVOD[i][1][1] - muonNEVOD[i][1][0]) *
                   (muonNEVOD[i][1][1] - muonNEVOD[i][1][0]) +
               (muonNEVOD[i][2][1] - muonNEVOD[i][2][0]) *
                   (muonNEVOD[i][2][1] - muonNEVOD[i][2][0]) +
               (muonNEVOD[i][3][1] - muonNEVOD[i][3][0]) *
                   (muonNEVOD[i][3][1] - muonNEVOD[i][3][0]));
  ...
  for (G4int j = 0; j < feuCount; j++) {
    ...
    if (photoelCount > 0) {
      ...
      for (G4int i = 0; i < photoelCount; i++) { // Amp(Q)
        ...
        if (photoelCount > 1)
          Q = normDistribution(Q, 2.8); // delta_1e*A_1e = 0.7*4.0 = 2.8
        amplitude_d += Q;
      }
      if (amplitude_d < 1.0)
        amplitude_d = 1;
  ...
  file_writer.writeEvent(event, amplitudeKSM, hSM, nTrackSMX, nTrackSMY,
                         nTrackSM, edepCountSCT1);

  fTimer->Stop();

  eventCount++;

  G4cout << eventCount << '\t' << runNum << '\t' << eventNum << '\t' << theta
         << '\t' << phi << '\t' << energyDepNEVOD << '\t' << particleCountNEVOD
         << '\t' << muonTrackLengthNEVOD << '\t' << muonCountNEVOD << '\t'
         << muonEnergyNevodStart / 1000. << '\t' << muonEnergyNevodEnd / 1000.
